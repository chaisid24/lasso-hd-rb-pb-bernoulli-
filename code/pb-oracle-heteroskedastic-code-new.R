...
# this code is meant for heteroskedastic errors case, and only Oracle based, PB based and Debiased Lasso based CIs are obtained in this setting 
# the details are very similar to the code for the homoskedastic case, as most of the code is similar
# some parts of the PB based pivotal quantities differ from the homoskedastic case
# only additional comments are included here
...
# declaring the k.fac sequence
k0.seq= seq(4, 0.375, by= -0.125)

# this code has been run only for error type 2 (= chi.square errors), but one can use err.type = 1
